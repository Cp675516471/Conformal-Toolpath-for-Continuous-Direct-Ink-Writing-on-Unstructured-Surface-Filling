...
%导入切片曲面模型
name = '.\03_Surfaces\Thumb_Surfaceoffset05.obj';
options.name = name; % useful for displaying
[F_Surface,X_Surface] = read_obj(name);
showMesh(F_Surface,X_Surface);

%Tips Location (第一个点为原点，第二个点为X轴，逆时针排列)★（后续用于机械臂的坐标转换）
Tips = [0,0,0;
       24,0,0;
       24,47,0];
MName = 'Thumb_Surfaceoffset05';
pathMethod = 2;%路径方法%★
...
% showMesh(ModelNozzle.faces,ModelNozzle.vertices);
%导入切片曲面模型
name = '.\03_Surfaces\Index_Finger_surfaceoffset05.obj';
options.name = name; % useful for displaying
[F_Surface,X_Surface] = read_obj(name);
showMesh(F_Surface,X_Surface);
...
Tips = [0,0,0;
       20,0,0;
       20,47,0];
MName = 'Index_Finger_surfaceoffset05';
pathMethod = 2;%路径方法%★
...
% showMesh(ModelNozzle.faces,ModelNozzle.vertices);
%导入切片曲面模型
name = '.\03_Surfaces\Midle_Finger_Surfaceoffset05.obj';
options.name = name; % useful for displaying
[F_Surface,X_Surface] = read_obj(name);
showMesh(F_Surface,X_Surface);
...
Tips = [0,0,0;
       20,0,0;
       20,55,0];
MName = 'Midle_Finger_Surfaceoffset05';
pathMethod = 2;%路径方法%★
NearStartPoint = [21 10 -0.5];%路径起点近点%★
%% 1.1.文件导入 （无名指，基底＋曲面，需要V1版本一笔画路径）
% 绘制世界坐标系
plot_coordinate_frame(eye(3), [0;0;0], 15, '世界坐标系');
%导入基底模型 
...
% showMesh(ModelNozzle.faces,ModelNozzle.vertices);
%导入切片曲面模型
name = '.\03_Surfaces\Ring_Finger_Surfaceoffset05.obj';
options.name = name; % useful for displaying
[F_Surface,X_Surface] = read_obj(name);
showMesh(F_Surface,X_Surface);
...
Tips = [0,0,0;
       18,0,0;
       18,55,0];
MName = 'Ring_Finger_Surfaceoffset05';
pathMethod = 3;%路径方法%★
...
% showMesh(ModelNozzle.faces,ModelNozzle.vertices);
%导入切片曲面模型
name = '.\03_Surfaces\little_finger_Surfaceoffset05.obj';
options.name = name; % useful for displaying
[F_Surface,X_Surface] = read_obj(name);
showMesh(F_Surface,X_Surface);
...
Tips = [0,0,0;
       16,0,0;
       16,35,0];
MName = 'little_finger_Surfaceoffset05';
pathMethod = 2;%路径方法%★
NearStartPoint = [14 16 2.3];%路径起点近点%★
%% 1.1.文件导入 （手掌MINI，基底+曲面）
% 绘制世界坐标系
plot_coordinate_frame(eye(3), [0;0;0], 15, '世界坐标系');
%导入基底模型 
ModelBase = stlread('.\01_BaseModels\PalmBaseMINI.STL');%读入你的stl文件，得到三角网格
F_Base = ModelBase.faces;
X_Base = ModelBase.vertices;
showMesh(ModelBase.faces,ModelBase.vertices);
%导入针头模型 
...
% showMesh(ModelNozzle.faces,ModelNozzle.vertices);
%导入切片曲面模型
name = '.\03_Surfaces\PalmSurfaceMINIoffset05.obj';
options.name = name; % useful for displaying
[F_Surface,X_Surface] = read_obj(name);
showMesh(F_Surface,X_Surface);
...
MName = 'PalmSurfaceMINIoffset05';
pathMethod = 2;%路径方法%★
NearStartPoint = [18 12 0];%路径起点近点%★
%% 1.1.文件导入 （手腕MINI，基底＋曲面）
% 绘制世界坐标系
plot_coordinate_frame(eye(3), [0;0;0], 15, '世界坐标系');
%导入基底模型 
ModelBase = stlread('.\01_BaseModels\WristBaseMINI.STL');%读入你的stl文件，得到三角网格
F_Base = ModelBase.faces;
X_Base = ModelBase.vertices;
showMesh(ModelBase.faces,ModelBase.vertices);
%导入针头模型 
...
% showMesh(ModelNozzle.faces,ModelNozzle.vertices);
%导入切片曲面模型
name = '.\03_Surfaces\WristSurfaceMINIoffset05.obj';
options.name = name; % useful for displaying
[F_Surface,X_Surface] = read_obj(name);
showMesh(F_Surface,X_Surface);
...
Tips = [0,0,0;
       ...
       16.5,38.5,0];
pathMethod = 2;%路径方法%★
MName = 'WristSurfaceMINIoffset05';
pathMethod = 2;%路径方法%★
NearStartPoint = [17 18 4];%路径起点近点%★
%% 1.1.文件导入 （手掌，基底＋曲面）
% 绘制世界坐标系
plot_coordinate_frame(eye(3), [0;0;0], 15, '世界坐标系');
%导入基底模型 
ModelBase = stlread('.\01_BaseModels\PalmBaseV5.STL');%读入你的stl文件，得到三角网格
F_Base = ModelBase.faces;
X_Base = ModelBase.vertices;
showMesh(ModelBase.faces,ModelBase.vertices);
%导入针头模型 
...
% showMesh(ModelNozzle.faces,ModelNozzle.vertices);
%导入切片曲面模型
name = '.\03_Surfaces\PalmSurfaceoffset05.obj';
options.name = name; % useful for displaying
[F_Surface,X_Surface] = read_obj(name);
showMesh(F_Surface,X_Surface);
...
Tips = [0,0,0;
       65,0,0;
       0,55,0];
MName = 'PalmSurfaceoffset05';
pathMethod = 2;%路径方法%★
NearStartPoint = [67 11 8.5];%路径起点近点%★
%% 1.1.文件导入 （手腕，基底+曲面）
% 绘制世界坐标系
plot_coordinate_frame(eye(3), [0;0;0], 15, '世界坐标系');
%导入基底模型 
ModelBase = stlread('.\01_BaseModels\WristBaseV5.STL');%读入你的stl文件，得到三角网格
F_Base = ModelBase.faces;
X_Base = ModelBase.vertices;
showMesh(ModelBase.faces,ModelBase.vertices);
%导入针头模型 
...
% showMesh(ModelNozzle.faces,ModelNozzle.vertices);
%导入切片曲面模型
name = '.\03_Surfaces\WristBaseV5Surface.obj';
options.name = name; % useful for displaying
[F_Surface,X_Surface] = read_obj(name);
showMesh(F_Surface,X_Surface);
...
Tips = [0,0,0;
       80,0,0;
       80,55,0];
MName = 'WristSurfaceV5';
pathMethod = 2;%路径方法%★
NearStartPoint = [52, 71, -4.7];%路径起点近点%★
%% 1.1.文件导入 （复杂孔洞，基底+曲面 需要V1版本一笔画路径）
% 绘制世界坐标系
plot_coordinate_frame(eye(3), [0;0;0], 15, '世界坐标系');
%导入基底模型 
ModelBase = stlread('.\01_BaseModels\20251021printingTestSurfaceBase.STL');%读入你的stl文件，得到三角网格
F_Base = ModelBase.faces;
X_Base = ModelBase.vertices;
showMesh(ModelBase.faces,ModelBase.vertices);
%导入针头模型 
...
% showMesh(ModelNozzle.faces,ModelNozzle.vertices);
%导入切片曲面模型
name = '.\03_Surfaces\20251021printingTestSurfaceoffset05.obj';
options.name = name; % useful for displaying
[F_Surface,X_Surface] = read_obj(name);
showMesh(F_Surface,X_Surface);
...
Tips = [0,0,0;
       84,0,0;
       84,84,0];
MName = 'TestSurfaceoffset05';
pathMethod = 3;%路径方法%★
NearStartPoint = [84,0,0];%路径起点近点%★
%% 1.1 文件导入 （V形面，基底+曲面）
...
offsetDistance = 0.45;
[S,normals] = surfaceOffset(F_Surface,X_Surface,offsetDistance);
...
% 生成路径
stpLength = 0.45;%路径间距%★
startvalue = stpLength*1.5;
%pathMethod = input('请移动机械臂TCP到第一个尖点 (1-通用抬针, 2-一笔画V0, 3-一笔画V1: ');
switch pathMethod
    case 1
       % 存在抬针
       lift_height = 5;
       [ALLLLLLOnePath_new] = HeatField2PathV0(name,stpLength,lift_height,startvalue);
    case 2 
        %一笔螺旋V0（只有一个局部点的情况）
        [G_ALLLLLLOnePath_new,ALLLLLLOnePath_new] = MyOnePathGenaratorV0(name,stpLength,NearStartPoint,startvalue);%生成路径点
        ALLLLLLOnePath_new(:,4) = 1;
    case 3 
        %一笔螺旋V1（只有多个局部点的情况，不是很稳定需要进一步优化）
        figure;
        [G_ALLLLLLOnePath_new,ALLLLLLOnePath_new] = MyOnePathGenaratorV1(name,stpLength,NearStartPoint,startvalue);% 这个版本仍然存在一个BUG需要后续进行优化
        ALLLLLLOnePath_new(:,4) = 1;
end
Flag = ALLLLLLOnePath_new(:,4);
figure;
hold on;
plot3(ALLLLLLOnePath_new(:,1),ALLLLLLOnePath_new(:,2),ALLLLLLOnePath_new(:,3),'b-');
scatter3(ALLLLLLOnePath_new(Flag==0,1),ALLLLLLOnePath_new(Flag==0,2),ALLLLLLOnePath_new(Flag==0,3),"red",SizeData=1);
...
% 为路径点添加法向
orderALLPoints = ALLLLLLOnePath_new(:,1:3);
point_normals = assignPointNormalsFast(orderALLPoints, S.vertices, S.faces, normals);
% 将法向信息添加到路径中
orderALLPoints_with_normals = [orderALLPoints, point_normals];
figure;
%法向量显示
plot3(orderALLPoints_with_normals(:,1), orderALLPoints_with_normals(:,2), ...
      orderALLPoints_with_normals(:,3),'b-');
hold on;
quiver3(orderALLPoints_with_normals(:,1), orderALLPoints_with_normals(:,2), ...
        orderALLPoints_with_normals(:,3), orderALLPoints_with_normals(:,4), ...
        orderALLPoints_with_normals(:,5), orderALLPoints_with_normals(:,6));
%%% 3.1.路径稀疏采样处理========================================================
points = orderALLPoints_with_normals;

% 调用重采样函数
target_spacing = 1.5; % 根据实际情况调整
...
ResamplePath = [interp_points,interp_normals];
% 可视化结果
figure
hold on;
...
% [URGcode] = generateURGcodeABSV2(pathOnelayer,HomePos_mm,currentPos_XYZ_mm,RXYZ,V,Pressure);
% %文件保存
...
client = tcpclient('192.168.0.10', 30003);  % 使用新的tcpclient函数
client.ByteOrder = 'big-endian';  % 设置为大端模式
% 打开客户端与服务端连接，检查连接状态
[P_current_m,J_current_rad] = InfoFromRobot(client);
pressure = 135;%(0-400Kpa)%★
V = (0.1/88)*pressure;
analog_out_Value_set = V;
analog_out_port = 0;
msg_analog = ['set_standard_analog_out(',num2str(analog_out_port),',',num2str(analog_out_Value_set,'%.2f'),')\n'];
fprintf(client,msg_analog);
...
%         p3 = currentPos_XYZ_mm(3)/1000+pathOnelayer(i,3)/1000;
        ...
        if(i==1)
            ...
            URGcode = URGcode + sprintf('  movep(p[ %.5f, %.5f, %.5f, %.4f, %.4f, %.4f], a=1.2, v=%.5f, r=0.001)\n',p1+0.005,p2+0.005,HomePosition_m(3),Rx,Ry,Rz,V*2);
            URGcode = URGcode + sprintf('  set_analog_out(%d, %.2f)\n',analog_out_port,analog_out_Value_Mid);
            URGcode = URGcode + sprintf('  sleep(0.01)\n');
            URGcode = URGcode + sprintf('  set_digital_out(%d, %s)\n',digital_out_port,digital_out_Value_Mid);
            URGcode = URGcode + sprintf('  movep(p[ %.5f, %.5f, %.5f, %.4f, %.4f, %.4f], a=1.2, v=%.5f, r=0.001)\n',p1,p2,p3,Rx,Ry,Rz,V);
